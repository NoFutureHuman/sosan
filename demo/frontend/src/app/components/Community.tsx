...
export function Community() {
  ...
  const userEmail = getStoredUser()?.email?.toLowerCase() ?? null;

  const isOwnPost = (post: BoardPost) => userEmail !== null && post.authorEmail === userEmail;
  const isOwnComment = (authorEmail: string) => userEmail !== null && authorEmail === userEmail;
  ...
  useEffect(() => {
    refreshBoard();
    const onAuthChange = () => refreshBoard();
  ...
  const handleDeletePost = (postId: number) => {
    ...
    if (!deleteUserPost(postId)) return;
  ...
  const handleDeleteComment = (commentId: string) => {
    ...
    if (!deleteUserComment(commentId)) return;
    refreshBoard();
                ...
                >
                  ...
                  <div className="py-4 pl-3 min-w-0">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <h4 className="truncate text-gray-200 group-hover:text-primary transition-colors flex-1 min-w-0" style={{ fontSize: "0.92rem", fontWeight: 600 }}>
                        {post.title}
                      </h4>
                      {isOwnPost(post) && (
                        <button
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-start gap-2 mb-1">
                        <p className="text-gray-200 group-hover:text-primary transition-colors truncate flex-1 min-w-0" style={{ fontSize: "0.9rem", fontWeight: 600 }}>{post.title}</p>
                        {isOwnPost(post) && (
                          <button
                            type="button"
                            onClick={(e) => { e.stopPropagation(); handleDeletePost(post.id); }}
              ...
              <div className="flex items-center gap-2 shrink-0">
                {isOwnPost(selectedPost) && (
                  <button
                    ...
                    className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-red-400 hover:bg-red-500/10 transition-colors text-sm"
                  >
                  ...
                  postComments.map((c) => (
                    <div key={c.id} className="rounded-lg px-3 py-2 bg-white/5">
                      <div className="flex items-start justify-between gap-2">
                        <p className="text-gray-200 text-sm flex-1">{c.content}</p>
                        {isOwnComment(c.authorEmail) && (
                          <button
                            type="button"
                      ...
                      <p className="text-gray-500 text-xs mt-1">
                        {c.authorName || "사장님"} · {c.createdAt.slice(0, 16).replace("T", " ")}
                      </p>
                    </div>
                  ))
                )}
