...
export function formatRegionAddress(address: RegionAddressValue): string {
  return address.sigungu ? `${address.sido} ${address.sigungu}` : address.sido;
}
...
export function DetailedStartupQuestionnaire({ onBack, onComplete }: Props) {
  ...
  const [address, setAddress] = useState<RegionAddressValue>({ sido: "", sigungu: "" });
  ...
  const handleNext = () => {
    if (q.type === "address") {
      setVal("region", formatRegionAddress(address));
    }
    if (step < TOTAL - 1) {
      setStep(s => s + 1);
    } else {
  ...
  return (
      <div style={PAGE_BG}>
        <div style={{ maxWidth: 640, margin: "0 auto", padding: "32px 20px" }}>
          ...
          {q.type === "address" ? (
              <RegionAddressInput address={address} onChange={setAddress} />
          ) : (
...
export function RegionAddressInput({
  ...
  // ★ 여기가 빨간줄 해결 핵심 (타입을 string[]으로 강제 지정)
  const keys = Object.keys(sigunguData) as string[];

  const sidoList = useMemo(() => [...new Set(keys.map(k => k.split("/")[0]))].sort(), [keys]);
