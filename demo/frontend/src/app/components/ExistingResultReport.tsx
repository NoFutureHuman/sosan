...
function resolveAppliedCategories(
  selectedCategories: string[],
  answers: Record<string, string | string[]>,
  aiResult: unknown,
): string[] {
  const fromProp = selectedCategories.map((c) => c.trim()).filter(Boolean);
  if (fromProp.length > 0) return fromProp;

  const fromAnswers = Array.isArray(answers.selectedCategories)
    ? (answers.selectedCategories as string[]).map((c) => c.trim()).filter(Boolean)
    : [];
  if (fromAnswers.length > 0) return fromAnswers;

  const result = aiResult as { selectedCategories?: string[] } | null;
  const fromResult = Array.isArray(result?.selectedCategories)
    ? result.selectedCategories.map((c) => c.trim()).filter(Boolean)
    : [];
  return fromResult;
}

function filterAiResultByCategories(aiResult: unknown, appliedCategories: string[]): unknown {
  if (!aiResult || appliedCategories.length === 0) return aiResult;
  const match = (label: string) =>
    appliedCategories.some(
      (c) => normalizeCategoryLabel(c) === normalizeCategoryLabel(label),
    );
  const src = aiResult as {
    categoryInsights?: Array<{ category?: string }>;
    improvements?: Array<{ category?: string }>;
  };
  const next = { ...(aiResult as object) } as {
    categoryInsights?: Array<{ category?: string }>;
    improvements?: Array<{ category?: string }>;
  };
  if (Array.isArray(src.categoryInsights)) {
    next.categoryInsights = src.categoryInsights.filter((item) =>
      match(item.category ?? ""),
    );
  }
  if (Array.isArray(src.improvements)) {
    next.improvements = src.improvements.filter((item) =>
      match(item.category ?? ""),
    );
  }
  return next;
...
function getCategoryInsightFromAi(
  aiResult: any,
  category: string,
  categoryIndex = -1,
) {
  const items = aiResult?.categoryInsights ?? [];
  if (!Array.isArray(items) || items.length === 0) return undefined;

  const target = normalizeCategoryLabel(category);
  let found = items.find(
    (item: { category?: string }) =>
      normalizeCategoryLabel(item.category ?? "") === target,
  );
  if (!found) {
    found = items.find((item: { category?: string }) => {
      const norm = normalizeCategoryLabel(item.category ?? "");
      return norm.includes(target) || target.includes(norm);
    });
  }
  if (!found && categoryIndex >= 0 && categoryIndex < items.length) {
    found = items[categoryIndex];
  }
  return found as
    | { summary?: string; signals?: string[]; actions?: string[] }
    | undefined;
}
...
function buildExistingSolutionFromAi(aiResult: any): AiData | null {
  ...
  if (summary) {
    const actions = [
      ...(aiResult.actionPlan?.flatMap((plan: { tasks?: string[] }) => plan.tasks ?? []) ??
        []),
      ...(aiResult.riskFactors?.map((risk: { mitigation?: string }) => risk.mitigation) ??
        []),
    ]
      .filter(Boolean)
      .slice(0, 6);
    ...
    const detailed =
      [
        ...(aiResult.actionPlan?.map(
          (plan: { period?: string; focus?: string; tasks?: string[] }) =>
            `[${plan.period ?? ""}] ${plan.focus ?? ""}\n${(plan.tasks ?? [])
              .map((task) => `- ${task}`)
              .join("\n")}`,
        ) ?? []),
        ...(aiResult.riskFactors?.map(
          (risk: { category?: string; description?: string; mitigation?: string }) =>
...
export function ExistingResultReport({
  ...
  kamisData?: KamisMarketData | null;
  onReset: () => void;
  onGoMain: () => void;
  selectedCategories?: string[];
}) {
  ...
  const categoryPages = useMemo(
    () =>
      detailStats.map((stat, statIndex) => {
        const aiCategory = getCategoryInsightFromAi(
          filteredAiResult,
          stat.label,
          statIndex,
        );
        const signals =
          (aiCategory?.signals ?? []).filter(Boolean).length > 0
            ? (aiCategory?.signals ?? [])
            : (aiCategory?.actions ?? []).slice(0, 4);
        return {
          ...stat,
  ...
  const currentInsight = useMemo(() => {
    const label = categoryPages[Math.max(0, stage - 1)]?.label ?? "";
    const aiCategory = getCategoryInsightFromAi(
      filteredAiResult,
      label,
      Math.max(0, stage - 1),
    ...
    if (aiCategory) {
      const bullets =
        (aiCategory.signals ?? []).length > 0
          ? (aiCategory.signals ?? [])
          : (aiCategory.actions ?? []).slice(0, 4);
      return {
        title: `${label} 핵심 인사이트`,
        desc: aiCategory.summary ?? "",
        bullets,
        ctaLabel: "",
        ctaPath: "",
      };
    }
    if (label === "가격 경쟁력" && kamisReportItems.length > 0) {
      return {
        title: "요즘 시세",
        desc: "KAMIS·상권 데이터 기준 주요 품목입니다.",
        bullets: kamisReportItems.map(
          (item) => `${item.name} · ${item.price} (${item.trend})`,
        ),
        ctaLabel: "시세 사이트 바로가기",
        ctaPath: "/market-price",
  ...
  const categorySignals = useMemo(
    () =>
      stage >= 1 && stage <= CATEGORY_STAGE_COUNT
        ? categoryPages[stage - 1]?.signals ?? []
        : [],
    [stage, CATEGORY_STAGE_COUNT, categoryPages],
  );
  ...
  useEffect(() => {
    setIsAiLoading(!resolvedAiData);
  }, [stage, resolvedAiData]);
    ...
    >
      <div className="max-w-[1100px] mx-auto">
        ...
        <div className="mb-8">
          ...
          />
          {commercialCtx && stage === 0 && (
            <p
              className="mt-4 text-xs"
              style={{ color: "rgba(255,255,255,0.45)" }}
            >
              상권 실거래·임대 시세 데이터가 분석에 반영되었습니다 (
              {commercialCtx.latestYearMonth} · {commercialCtx.sampleCount}건)
            </p>
          )}
          {sbizData && stage === 0 && sbizData.stores.length > 0 && (
            <p className="mt-2 text-xs" style={{ color: "rgba(255,255,255,0.45)" }}>
              인근 동종 업체 {sbizData.stores.length}곳 데이터를 참고했습니다.
            </p>
          )}
          {stage > 0 && (
            <div
                  ...
                  <p className="text-zinc-300 leading-8 max-w-3xl mb-8">
                    {appliedCategories.length > 0
                      ? (
                        <>
                          선택하신 카테고리(
                          {appliedCategories.join(", ")}) 기준으로 질문 응답과 운영 맥락을
                          정리해 실행안을 안내합니다. 아래 카드에서{" "}
                          <b>원하는 항목을 클릭해 바로 확인</b>하거나 화살표를 눌러
                          순서대로 볼 수 있습니다.
                        </>
                      )
                      : "저장된 카테고리 선택 정보가 없습니다. 새로 분석을 실행해 주세요."}
                  </p>
            ...
            <div className="bg-gradient-to-br from-[#121214] to-emerald-400/20 border border-[#27272a] rounded-[20px] p-6 sm:p-8">
              ...
              {isAiLoading ? (
                ...
                <div className="h-[240px] flex flex-col items-center justify-center text-zinc-400">
                  ...
                  AI 솔루션을 불러오지 못했습니다. 재분석을 시도해주세요.
                </div>
              )}
