...
export interface KamisMarketItem {
  name: string;
  priceText: string;
  trendText: string;
}
...
export function kamisItemsToReportBullets(
  items: KamisMarketItem[],
  limit = 3,
): { name: string; price: string; trend: string }[] {
  return items.slice(0, limit).map((item) => ({
    name: item.name,
    price: item.priceText,
    trend: item.trendText,
  }));
}

/** 분석·리포트용 외부 API 일괄 로드 */
export async function fetchExistingOwnerApiBundle(
  regionText: string,
  bizType: string,
) {
  const [commercial, sbiz, rone, bizinfo, kamis] = await Promise.all([
    fetchCommercialContext(regionText),
    fetchSbizStores(regionText, bizType),
    fetchRoneRentData(regionText),
    fetchBizinfoSupport(bizType),
    fetchKamisMarketPrices(bizType),
  ]);
  return { commercial, sbiz, rone, bizinfo, kamis };
}
